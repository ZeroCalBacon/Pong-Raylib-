// PongGameVaultTec.cpp
#include "PongGameVaultTec.h"
#include "MathFunction.h"
#include <math.h>

//inherits from PongGame, this is "Vault-Tec" game style
//changes theme based on Fallout Vault-Tec
//blue background, yellow objs

PongGameVaultTec::PongGameVaultTec() : PongGame() {
...
PongGameVaultTec::~PongGameVaultTec() {}

void PongGameVaultTec::Update() {
    switch (gameState) {
        case COUNTDOWN:
            UpdateCountdown();
            break;
...
void PongGameVaultTec::Draw() {
    ...
    if (gameState != WINNER_GAMEOVER && gameState != TRY_AGAIN) {
        ...
        DrawText(TextFormat("%d", playerScore), screenWidth / 4, 20, 40, YELLOW);
        DrawText(TextFormat("%d", botScore), 3 * screenWidth / 4, 20, 40, YELLOW);

        for (int i = 0; i < GetScreenHeight(); i += 20) {
            DrawRectangle(screenWidth / 2, i, 4, 10, YELLOW);
        }

        DrawRectangleRec(playerPaddle, YELLOW);
        DrawRectangleRec(botPaddle, YELLOW);

        DrawCircle(ball.x + ballRadius, ball.y + ballRadius, ballRadius, YELLOW);
        }
    ...
    if (gameState == WINNER_GAMEOVER) {
        ...
        DrawText("Game Over", gameOverTextPosition.x, gameOverTextPosition.y, gameOverFontSize, YELLOW);

        if (playerScore >= 5) {
            DrawText("Player Won", winnerTextPosition.x, winnerTextPosition.y, winnerFontSize, YELLOW);
        } else {
            DrawText("Bot Won", winnerTextPosition.x, winnerTextPosition.y, winnerFontSize, YELLOW);
        }
...
void PongGameVaultTec::InitializeGame() {
...
void PongGameVaultTec::ResetPositions() {
...
void PongGameVaultTec::UpdateCountdown() {
    countdownTimer--;

    if (countdownTimer > 0) {
        DrawText(TextFormat("%d", countdownTimer / 60 + 1), screenWidth / 2 - 20, screenHeight / 2 - 20, 40, YELLOW);
    } else {
        gameState = GAMEPLAY;
        InitializeGame();
    }
...
void PongGameVaultTec::UpdateTryAgain() {
    ...
    DrawText("Try Again?", tryAgainTextPosition.x, tryAgainTextPosition.y, tryAgainFontSize, YELLOW);

    Color optionColor = YELLOW;
...
void PongGameVaultTec::CheckGoal() {
    if (ball.x <= 0) {
        botScore++;
...
void PongGameVaultTec::UpdateResetGameplay() {
    countdownTimer--;
...
void PongGameVaultTec::MoveBotPaddle() {
...
void PongGameVaultTec::MoveBall() {
    ball.x += ballSpeed.x;
    ball.y += ballSpeed.y;
